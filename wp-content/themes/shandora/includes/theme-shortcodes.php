...
if(!function_exists('shandora_add_shortcodes')) {
	...
	function shandora_add_shortcodes() {
		...
		add_shortcode( 'listing', 'shandora_listing_shortcode' );
		add_shortcode( 'car-listing', 'shandora_car_listing_shortcode');
	}
...
function shandora_listing_shortcode( $attr ) {

	$attr = shortcode_atts(
		array(
			'property_type' => 'all',
			'property_location' => 'all',
			'property_feature' => 'all',
			'property_query' => 'latest',
			'numberposts' => 8,
		), $attr
	);
	...
	if( function_exists( 'shandora_render_builder_element_listing') ) {
		$o = shandora_render_builder_element_listing( $attr );
	}

	return $o;
}

function shandora_car_listing_shortcode( $attr ) {
	...
	if( function_exists( 'shandora_render_builder_element_car_listing') ) {
		$o = shandora_render_builder_element_car_listing( $attr );
	}
...
function shandora_affix_shortcode( $attr, $content = null ) {
    ...
    if( count($items) ){
		...
		foreach( $items as $item ) {
		    ...
		    $o .= '<li class="'.$class.'"><a href="#'.sanitize_title( $item[0] ).'">'. esc_attr( $item[0] ) .'<i class="bonicons bi-chevron-right"></i></a></li>';
		    $i++;
		}
