...
class Bon_IDX {
   ...
   static public $SearchSetupID = '';
	...
	function may_redirect($requested = false, $get = array()) {
		$requested = $requested ? $requested : esc_url( add_query_arg(array()) );
	...
	protected function add_template_actions() {
		...
		// Actions header
        add_action('bon_idx_before_results_content', array( $this, 'pro_profile_header'), 1 );
        // Enticement box
        add_action('bon_idx_before_results_content', array( $this, 'pro_enticement'), 7);

        add_action('bon_idx_admin_panel', array( $this, 'dsidx_main_js'), 1);
	...
	function dsidx_main_js() {
		echo $this->get_global('dsidx_js');
	}
  ...
  <?php
	...
	function process_results_listings($content = '') {
		...
		if (isset($matches[1])) {
			...
			/*if( !isset( $listings[1] ) || empty( $listings[1] ) ) {
				preg_match_all($this->get_regex('dsidx-listing'), $matches[1], $listings );
			}*/
	...
	function get_regexes() {
		...
		$regexes = apply_filters('bon_idx_get_regexes', array(
			...
			'dsidx_javascript_details2' => '/<script(?:.*?)>(\s+?dsidx.activate)<\/script>/ism',
            ...
            'login_form' => '/(<div.*?id="dsidx-login".*?>.*?<\/div>)/ism',
            'forgot_form' => '/(<div.*?id="dsidx-forgotpassword-confirm".*?>.*?<\/div>)/ism',
            'reset_form' => '/(<div.*?id="dsidx-passwordreset".*?>.*?<\/div>)/ism',
            'register_form' => '/(<div.*?id="dsidx-register".*?>.*?<\/div>)/ism',
            'dsidx_js' => '/<script\s*src="(?P<src>[^"]+)"\s*data-handle="dsidx"><\/script>/ism'
		));
	...
	function pro_enticement() {
        
        if (!$this->get_global('pro_enticement')) {
            return;
        }
        
        // If the enticement is shown, we don't want to show this. There are results, but they aren't shown on purpose.
        remove_action( 'bon_idx_no_results', array( $this, 'template_no_results') );
        
        do_action('bon_idx_before_pro_enticement');
        
        	$this->idx_include(apply_filters('bon_idxs_pro_enticement', 'pro-enticement.php'), false);

        do_action('bon_idx_after_pro_enticement');
    }
	...
	function process_globals($content = null) {
		...
		$pe = $this->helper->strip_spaces($qp->find('.dsidx-enticement')->xhtml() );
		$this->helper->set_global( 'pro_enticement', $pe );		
        /*if (!empty($matches['data'])) {
            
            preg_match_all($this->get_regex('list_explode'), $matches['data'], $property_images);
            ...
            if ($this->helper->is_listing()) {
                ...
                preg_match( $this->get_regex('pro_enticement_count_and_location'), $matches['data'], $count_and_location);
                
                if (!empty($count_and_location['area'])) {
                    $type = 'area';
                } elseif (!empty($count_and_location['zip'])) {
                    $type = 'zip';
                } elseif (!empty($count_and_location['city'])) {
                    $type = 'city';
                } elseif (!empty($count_and_location['community'])) {
                    $type = 'community';
                } elseif (!empty($count_and_location['tract'])) {
                    $type = 'tract';
                } else {
                    $type = 'search';
                }
                
                $enticement['count']    = $count_and_location['count'];
                $enticement['location'] = $count_and_location["{$type}"];
            }
            
            $enticement['type']    = $type;
            $enticement["{$type}"] = $enticement['location'];
            
            $this->helper->set_global('pro_enticement', $enticement);	
        }
        */
		...
		if ($this->helper->is_listing()) {
	        ...
	        preg_match($this->get_regex('dsidx_js'), $content, $matches);
	        if( isset( $matches[1] ) ) {
	        	$this->helper->set_global('dsidx_js', "\n" . '<script src="' . $matches[1] . '" data-handle="dsidx"></script>');
	        }
		...
		$log = $this->helper->strip_spaces($qp->find('#dsidx-login')->xhtml());
	    $this->helper->set_global('login_form', $log);

	    $reg = $this->helper->strip_spaces($qp->find('#dsidx-register')->xhtml());
	    $this->helper->set_global('register_form', $reg);

	    $forgot = $this->helper->strip_spaces($qp->find('#dsidx-forgotpassword-confirm')->xhtml());
	    $this->helper->set_global('forgot_form', $forgot);

	    $reset = $this->helper->strip_spaces($qp->find('#dsidx-passwordreset')->xhtml());
	    $this->helper->set_global('reset_form', $reset);
		
		//$html = preg_replace('~<script[^>]*>\K[^<]*(?=</script>)~i', '', $content);
		 // Javascript Details
        preg_match( '/(\s+?dsidx.activate.*?)<\/script>/ism', $content, $matches);
	...
	function get_pagenum_link($page = null) {
		...
		if (is_null($page)) {
			return preg_match('/page-[0-9]+/ism', $wp->request) ? preg_replace('/\/page-[0-9]+/ism', '/%_%', esc_url( add_query_arg(array()) ) ) : esc_url( add_query_arg(array(), site_url(untrailingslashit($wp->request) . '/%_%')) );
		} else {
			$append = !empty($page) ? '/page-' . $page : '/';
			return preg_match('/page-[0-9]+/ism', $wp->request) ? preg_replace('/\/page-[0-9]+/ism', $append, esc_url( add_query_arg(array()) ) ) : esc_url( add_query_arg(array(), site_url(untrailingslashit($wp->request) . $append)) );
		}
	}
