...
function shandora_get_listing_price($echo = true, $secondary = false ) {
	global $post;

	$priceastext = shandora_get_meta( $post->ID, 'listing_pricetext', true);
	if( $priceastext == '1' && $secondary == false ) {
		$o = bon_get_option('price_text', __('Call For Quote', 'bon'));
	} else {
		...
		if( $secondary == false ) {
			...
			$price = shandora_get_meta($post->ID, 'listing_price_sec', true);
		}
		...
		if( $secondary == true ) {
			$o .= ' ' . bon_get_option( 'secondary_price_end' );
		}
	}
...
function shandora_get_price_meta( $postID, $secondary = false ) {
	...
	if( $secondary == false ) {
		...
		$price = shandora_get_meta( $postID, 'listing_price_sec', true);
	}
...
function shandora_format_price( $value = 0 ) {
	if( !$value ) {
		return;
	}
	...
	if( is_float( $value ) ) {
		if($price_format == 'comma') {
			$price = esc_attr( number_format( (double) $value, 2, '.', ',' ) );
		} else {
			$price = esc_attr( number_format( (double) $value, 2, ',', '.' ) );
		}
	} else {
		if($price_format == 'comma') {
			$price = esc_attr( number_format( (double) $value, 0, '.', ',' ) );
		} else {
			$price = esc_attr( number_format( (double) $value, 0, ',', '.' ) );
		}
	}
...
function shandora_listing_post_per_page( $query ) {
    if(!is_admin()) {
    	...
    	if( ( $query->is_tax( get_object_taxonomies('listing') ) || $query->is_tax( get_object_taxonomies('car-listing') ) || $query->is_tax( get_object_taxonomies('boat-listing') ) ) && $query->is_main_query() ) {
			...
			} else if ( $query->is_tax() && in_array( $query->tax_query->queries[0]['taxonomy'], $car_tax ) ) {

					$query->set('post_type', 'car-listing');

			} else if( $query->is_tax()  && in_array( $query->tax_query->queries[0]['taxonomy'], $boat_tax ) ) {
