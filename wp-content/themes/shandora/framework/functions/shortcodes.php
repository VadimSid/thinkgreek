...
function bon_add_shortcodes() {
	...
	add_shortcode( 'front-end-post', 'bon_front_end_post' );
}
...
function bon_entry_icon_shortcode( $attr ) {
	...
	switch ($format) {
		...
		case 'aside':
			$o .= '<i class="bonicons bi-pencil"></i>';
		break;
		
		default:
			$o .= '<i class="bonicons bi-file"></i>';
		break;
	}
...
function bon_front_end_post( $attr ) {
	...
	foreach( $post_types as $post_type ) {
		if( post_type_supports( $post_type, 'front-end-post' ) && $post_type != 'attachment' ) {
			$supported_post_type[] = $post_type;
		}
	}

	if( !in_array( $attr['post_type'], $supported_post_type ) ) {
		return __('The post type assigned is not supporting front end post', 'bon');
	}
