...
function bon_get_default_widget_menu_args() {
	/* Set up some default sidebar arguments. */
	$defaults = array(
		'before_widget' => '<li id="%1$s" class="bon-menu-widget widget %2$s">',
		'after_widget'  => '</div></li>',
		'before_title'  => '<h3 class="bon-menu-widget-title">',
		'after_title'   => '</h3><div class="bon-menu-widget-item">',
	);
		...
		foreach( $sidebars as $sidebar ) {
			...
			if( !empty($sidebar['sidebar_name']) ) {
				...
				if(array_key_exists($id, $wp_registered_sidebars)) {
					...
					/* Allow developers to filter the default sidebar arguments. */
					if( isset( $sidebar['is_menu'] ) ) {
						$defaults = apply_filters( bon_get_prefix() . 'sidebar_defaults_menu', bon_get_default_widget_menu_args(), $args );
					} else {
