...
function bon_page_navi($args = array()) {
    ...
    if ($start_page >= 2 && $pages_to_show < $max_page && $args['first_last_button'] === true  ) {
        $first_page_text = __('First','bon');
        $output .= '<li class="bon-first-page-link '.$first_class.'"><a href="'.get_pagenum_link().'" title="'.$first_page_text.'"><i class="bonicons bi-angle-double-left"></i></a></li>';
    }

    if( $args['prev_next_button'] === true ) {
    ...
    if ($end_page < $max_page && $args['first_last_button'] === true ) {
        $last_page_text = __('Last','bon');
        $output .= '<li class="bon-last-page-link '.$last_class.'"><a href="'.get_pagenum_link($max_page).'" title="'.$last_page_text.'"><i class="bonicons bi-angle-double-right"></i></a></li>';
    }
    $output .= '</ul></div>'.$after."";
