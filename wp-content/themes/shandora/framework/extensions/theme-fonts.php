...
final class Theme_Fonts {
	 ...
	 * @var    string
	 */
	private $google_api = 'https://www.googleapis.com/webfonts/v1/webfonts';
	 ...
	 * @var    string
	 */
	public $google_transient = 'theme_fonts_google_cache';
	...
	public function __construct() {
		...
		add_action( 'theme_fonts_register', array( $this, 'register_google_font' ) );
	...
	protected function get_style_uri() {
		...
		if ( !empty( $supports[0] ) ) {
			...
			if ( !empty( $subset ) ) {
				...
				$font_subset = get_theme_mod( 'theme_font_subset' );
				$query_args['subset'] = $font_subset;
			}
	...
	public function customize_register( $wp_customize ) {
		...
		$wp_customize->add_setting(
			"theme_font_subset",
			array(
				'default'              => get_theme_mod( "theme_font_subset", '' ),
				'type'                 => 'theme_mod',
				'capability'           => 'edit_theme_options',
				'priority'             => $priority,
				'sanitize_callback'    => 'sanitize_key',
		...
		$wp_customize->add_control(
			"theme-fonts-subset",
			array(
				'label'    => __('Font Subset','bon'),
				'section'  => 'fonts',
				'settings' => 'theme_font_subset',
				'type'     => 'select',
				'choices'  => array(
					'' => __('-- Select Subset --', 'bon'),
					'cyrillic' => 'Cyrillic',
		...
		if( !empty( $google_fonts ) && is_array( $google_fonts ) ) :
	        ...
	        foreach( $google_fonts as $key => $value ) {
	        	...
	        	$args = array(
	        		'family' => $value['family'],
	        		'type' => 'google',
	        		'stack' => $this->get_google_font_stacks( $value['category'], $value['family'] ),
	        		'handle' => strtolower( str_replace(' ', '-', $value['family'] ) ),
	        		'label' => $value['family']
	        	);
	 ...
	 * @return array
	 */
	function fetch_google_fonts( $amount = 'all', $sort = 'alpha', $key = '' ) {

		$api = $this->google_api;

		$full_api = $api . '?sort=' . $sort;
		...
		$api_data = wp_remote_get( $full_api, array('sslverify'   => false ) );
        ...
        if ( is_wp_error( $api_data ) ) {
            ...
            $this->admin_notice = sprintf(__("Error fetching Google font: %s", "bon" ), $error_message );
		...
		if( $amount == 'all' ) {
            $font_lists = $font_list['items'];
        } else {
            $font_lists = array_slice( $font_list['items'], 0, $amount );
        }
	...
	function get_google_fonts() {
		...
		$google_fonts = get_transient( $this->google_transient );

		if ( false === ( $google_fonts ) || empty( $google_fonts ) ) {
		  // It wasn't there, so regenerate the data and save the transient

		   $api_key = bon_get_framework_option( 'bon_framework_google_api' );

		   if( empty( $api_key ) ) {
		   		$api_key = 'AIzaSyBIluei6yjgNvocIeSargnbOZHfSxy_HTg';
		   }
		   
		   $google_fonts = $this->fetch_google_fonts( 'all', 'alpha', $api_key );

		   	if( !empty( $google_fonts ) ) {
		   		set_transient($this->google_transient, $google_fonts, WEEK_IN_SECONDS * 4 ); // store for a month in transient
			}
		}
	...
	function get_google_font_stacks( $cat, $family ) {

		$stack = '';

		if( strpos( trim( $family ), ' ' ) !== false ) {
		    $stack = '"' . $family . '", ';
		}
		else {
		    $stack = $family . ', ';
