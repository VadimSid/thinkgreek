...
var bon_fee_postbox, tagBox, commentsBox, WPSetThumbnailHTML, WPSetThumbnailID, WPRemoveThumbnail, wptitlehint, makeSlugeditClickable, editPermalink;
...
(function($) {
	...
	bon_fee_postbox = {
		...
		add_postbox_toggles : function(page, args) {
			...
			$('.postbox h3, .postbox .handlediv').bind('click.bon_fee_postbox', function() {
				...
				if ( id ) {
					if ( !p.hasClass('closed') && $.isFunction(bon_fee_postbox.pbshow) )
						self.pbshow(id);
					else if ( p.hasClass('closed') && $.isFunction(bon_fee_postbox.pbhide) )
						self.pbhide(id);
				}
			});
...
jQuery(document).ready( function($) {
	...
	bon_fee_postbox.add_postbox_toggles(pagenow);
	...
	$submitButtons = $submitpost.find( ':button, :submit, a.submitdelete, #post-preview' ).on( 'click.edit-post', function( event ) {
		...
		$('form#bon-fee-post').off( 'submit.edit-post' ).on( 'submit.edit-post', function( event ) {
	...
	if ( $('#tagsdiv-post_tag').length ) {
		...
		$('.bon-fee-postbox').children('div.postbox').each(function(){
	...
	if ( $('#submitdiv').length ) {
		...
		$('#bon-fee-post').on( 'submit', function( event ) {
			if ( ! updateText() ) {
				event.preventDefault();
				$timestampdiv.show();
